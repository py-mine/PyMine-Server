...
class PlayTeams(Packet):
    ...
    def encode(self) -> bytes:
        ...
        if self.mode == 0:  # create team
            out += Buffer.pack_chat(Chat(self.data['team_display_name'])) + Buffer.pack('b', self.data['friendly_flags']) + \
                Buffer.pack_string(self.data['name_tag_visibility']) + Buffer.pack_string(self.data['collision_rule']) + \
                Buffer.pack_varint(self.data['team_color']) + Buffer.pack_chat(Chat(self.data['team_prefix'])) + \
                Buffer.pack_chat(Chat(self.data['team_suffix'])) + Buffer.pack_varint(len(self.data['entities'])) + \
            ...
            out += Buffer.pack_varint(len(self.data['entities'])) + b''.join(Buffer.pack_string(e) for e in self.data['entities'])
